(function () {
	...
	players.on('change', newVal => {
		newVal.forEach(player => {
			const cardEl = document.querySelector(`anaevent-dotafortress-card[player-class="${player.playerClass}"][index="${player.index}"]`);
	...
	teams.on('change', newVal => {
		for (let i = 0; i < 6; i++) {
			const redCardEl = document.querySelector(`#red anaevent-dotafortress-card:nth-child(${i + 1})`);
			const bluCardEl = document.querySelector(`#blu anaevent-dotafortress-card:nth-child(${i + 1})`);
